...
a<-read.csv("C:\\Users\\msgc\\Desktop\\14400scopus.csv")
...
b <- read.csv("C:\\Users\\msgc\\Desktop\\CitationOverviewall.csv")
...
summary(lm(no_self~author_binary, data=a))
summary(lm(no_self1~author_binary, data=a))

a<- a[a$author_number>0,]

a$inflation <- a$Cited.by -  a$no_self1

table(a$author_number>20)

# FALSE  TRUE 
# 14411     4 

a<-a[a$author_number<20,]

> 9.8293 -8.3210 
[1] 1.5083
summary(lm(no_self1~author_number, data=a))
summary(lm(Cited.by~author_binary, data=a[!is.na(a$no_self1),]))
summary(lm(inflation~author_number, data=a))

adoi <- data.frame(DOI=a$DOI[is.na(a$no_self1)],EID=a$EID[is.na(a$no_self1)], cited=a$Cited.by[is.na(a$no_self1)])
write.csv(adoi, "C:\\Users\\msgc\\Desktop\\forscopus.csv", row.names=F)
